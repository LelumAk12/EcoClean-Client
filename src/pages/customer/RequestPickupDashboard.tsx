...
const RequestPickupDashboard = () => {
  ...
  useEffect(() => {
    const fetchPickupRequests = async () => {
      ...
      try {
        ...
        const response = await fetch(`${backendUrl}/api/user/get-all-pickup-request`, {
          method: 'GET',
          headers: {
  ...
  const handleSaveEdit = async (requestId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/user/update-pickup-request/${requestId}`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const performDelete = async (requestId: string) => {
    try {
      const token = await getToken();
      const response = await fetch(`${backendUrl}/api/user/delete-pickup-request/${requestId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
